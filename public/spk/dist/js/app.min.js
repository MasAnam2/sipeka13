...
Vue.component('leftbar', __webpack_require__(38));
Vue.component('company-title', __webpack_require__(37));
var app = new Vue({
    el: '#app'
});
...
/* harmony default export */ __webpack_exports__["default"] = ({
	...
	methods: {
		moveModul: function (_moveModul) {
			function moveModul(_x, _x2, _x3) {
				return _moveModul.apply(this, arguments);
			}

			moveModul.toString = function () {
				return _moveModul.toString();
			};
	...
	created: function created() {
		...
		axios.get(base_url('/api/setting/menus')).then(function (resp) {
...
window.showFailedAlert = function (errorTxt) {
	var el = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '.failed-alert';
...
window.showSuccessAlert = function (successTxt) {
	var el = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '.success-alert';
...
window.pluginsRender = function () {
	if (!$('.real-time').length > 0) $('.breadcrumb').prepend('<li class="real-time"></li>');
	renderTimer();
	$(".select2").select2({
		tags: true
	});
...
window.iCheckRender = function () {
	...
	checkAll();
	unCheckAll();
	if ($('[data-menu="member"]').length > 0) accountEvent();
...
window.enterEvent = function () {
	$('input[type="text"].form-control, input[type="password"].form-control').on('keyup', function (e) {
		...
		if (e.keyCode == 13) f.find('#save').trigger('click');
	});
	$('textarea.form-control').on('keyup', function (e) {
		e.preventDefault();
		var f = $(this).parents('form');
		if (e.keyCode == 13 && e.ctrlKey == true) f.find('#save').trigger('click');
	});
...
window.eventMoneyInput = function () {
	$('.numeric-only').on('keydown', function (e) {
		numericOnly(this, e);
	});
};

window.numericOnly = function (el, e) {
	var k = e.key;
...
window.resetSelect = function (form) {
...
window.toggleDeleteSelected = function () {
...
window.statusEvent = function (el) {
	if ($(el).val() == 0) {
...
window.triggered = function (selector, onEvent) {
...
window.save = function (el, e) {
	var action = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'insert';
	...
	if (!window.MENYIMPAN) {
		...
		var saveCloseBtn = form.find('#save-close');
...
window.resetForm = function (form) {
	...
	if (form.find('[data-default-value]').length > 0) {
		var input = form.find('[data-default-value]');
...
window.update = function (e, el, formId) {
...
window.companyEdit = function (e) {
	e.preventDefault();
	var url = base_url('/company-profile/edit');
...
window.updateCompanyProfile = function (e, el) {
	return update(e, el, 'company-modal-edit-form', refreshCompanyProfile);
};

window.refreshCompanyProfile = function () {
...
window.calloutDanger = function (text) {
...
window.calloutSuccess = function (text) {
...
window.handleError = function (err, form) {
	if (err.response.status == 409) calloutDanger(err.response.data);else if (err.response.status == 422) {
		calloutDanger('Error occured!!!<br>Please check again field in form');
		var errors = err.response.data.errors ? err.response.data.errors : err.response.data;
		var input = void 0;
...
window.editAvatar = function (e) {
	e.preventDefault();
	$('#avatarModal').modal();
};

window.updateAvatar = function (e, el) {
	update(e, el, 'change-avatar-form', refreshAvatar);
	return false;
};

window.refreshAvatar = function () {
	axios.get(base_url('/profile/avatar-leftbar')).then(function (res) {
		$('#avatar-leftbar').html(res.data);
	});
	axios.get(base_url('/profile/avatar-image')).then(function (res) {
...
window.resetAllError = function () {
	$('.form-group').removeClass('has-error');
	$('span.help-block').find('strong').empty();
};

// SALARIES
window.checkSalary = function (el) {
	var form = $(el).parents('form');
	var employee = form.find('#employee').val();
	var month = form.find('#month').val();
	var year = form.find('#year').val();
	var salary_content = form.find('.salary-content');
	...
	axios.post(base_url('/salary/check'), {
		...
		onDownloadProgress: function onDownloadProgress(pe) {
			var percentCompleted = Math.floor(pe.loaded * 100 / pe.total);
...
window.salariesCallback = function () {
	if (window.location.pathname.includes('salaries')) {
		$('#employee').trigger('change');
	}
};

window.setClearSalary = function (el, event) {
	numericOnly(el, event);
	var form = $(el).parents('form');
	var basic_salary = Number(form.find('#basic_salary').val());
	console.log(form.find('#basic_salary').val());
	var allowance = Number(form.find('#allowance').val());
	var eat_cost = Number(form.find('#eat_cost').val());
	var transportation = Number(form.find('#transportation').val());
	var result_over_time = Number(form.find('#over_time_total').val());
	var loan = Number(form.find('#loan').val());
	var thr = Number(form.find('#thr').val());
	var reward = Number(form.find('#reward').val());
	var punishment = Number(form.find('#punishment').val());
	var clear = basic_salary + allowance + eat_cost + transportation + thr + reward + result_over_time - punishment;
...
window.deleteSelected = function (el) {
...
window.edit = function (id, url, modul) {
...
window.detail = function (id, url, modul) {
	var size = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : '';
...
window.modalSetup = function (title, content, size) {
...
window.remove = function (id, url) {
	...
	if (confir) {
		axios.delete(url, config).then(function (resp) {
...
window.filterAttendance = function (selector) {
	...
	if (selector == '#filter-date') {
		...
		if (time != 'all') moveModul('/attendance/filter/' + time, 'Attendances ' + selector.options[selector.selectedIndex].text);else moveModul('/attendances', 'Attendances');
	}
};

window.filterOverTime = function (selector) {
	var time = $(selector).val();
	if (selector == '#filter-date') {
		if (time) {
			time = time.replace('/', '-');
		...
		if (time != 'all') moveModul('/over_time/filter/' + time, 'Over Time ' + selector.options[selector.selectedIndex].text);else moveModul('/over_times', 'Over Time');
	}
};

window.filterSalaries = function () {
	var date = $('#filter_date_salaries').val().toString();
	if (!date.trim()) {
...
var BASE_URL = document.querySelector('[name="base-url"]').content;
...
Component.options.__file = "D:\\xampp\\htdocs\\sipeka_v1_3\\resources\\assets\\js\\components\\company-title.vue"
if (Component.esModule && Object.keys(Component.esModule).some(function (key) {return key !== "default" && key !== "__esModule"})) {console.error("named exports are not supported in *.vue files.")}
if (Component.options.functional) {console.error("[vue-loader] company-title.vue: functional components are not supported with templates, they should use render functions.")}
...
if (false) {(function () {
  ...
  module.hot.accept()
  if (!module.hot.data) {
    hotAPI.createRecord("data-v-7919ec7a", Component.options)
  } else {
    hotAPI.reload("data-v-7919ec7a", Component.options)
  }
})()}
...
Component.options.__file = "D:\\xampp\\htdocs\\sipeka_v1_3\\resources\\assets\\js\\components\\leftbar.vue"
if (Component.esModule && Object.keys(Component.esModule).some(function (key) {return key !== "default" && key !== "__esModule"})) {console.error("named exports are not supported in *.vue files.")}
...
if (false) {(function () {
  ...
  module.hot.accept()
  if (!module.hot.data) {
    hotAPI.createRecord("data-v-5c4b57a7", Component.options)
  } else {
    hotAPI.reload("data-v-5c4b57a7", Component.options)
  }
})()}
    ...
    return _c('li', {
      ...
      on: {
        "click": function($event) {
          ...
          _vm.moveModul(menu.link, menu.text, menu.modul)
        }
      }
    }, [_c('i', {
      class: menu.icon
...
if (false) {
  module.hot.accept()
  if (module.hot.data) {
     require("vue-hot-reload-api").rerender("data-v-5c4b57a7", module.exports)
  }
}
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    staticClass: "pull-left company-title"
  }, [_vm._v("\n\t" + _vm._s(_vm.name) + "\n")])
},staticRenderFns: []}
module.exports.render._withStripped = true
if (false) {
  module.hot.accept()
  if (module.hot.data) {
     require("vue-hot-reload-api").rerender("data-v-7919ec7a", module.exports)
  }
}
...
var update = __webpack_require__(42)("3a6fbf24", content, false);
// Hot Module Replacement
if(false) {
 // When the styles change, update the <style> tags
 if(!content.locals) {
   module.hot.accept("!!../../../../node_modules/css-loader/index.js!../../../../node_modules/vue-loader/lib/style-compiler/index.js?{\"id\":\"data-v-7919ec7a\",\"scoped\":false,\"hasInlineConfig\":true}!../../../../node_modules/vue-loader/lib/selector.js?type=styles&index=0!./company-title.vue", function() {
     var newContent = require("!!../../../../node_modules/css-loader/index.js!../../../../node_modules/vue-loader/lib/style-compiler/index.js?{\"id\":\"data-v-7919ec7a\",\"scoped\":false,\"hasInlineConfig\":true}!../../../../node_modules/vue-loader/lib/selector.js?type=styles&index=0!./company-title.vue");
